...
import { useWorkflowData, useCreateWorkflow, useAddMaterialToWorkflow } from '@/hooks/useDatabase';
...
const WorkflowSelector = ({ isOpen, material, onClose }: WorkflowSelectorProps) => {
  ...
  const addMaterialToWorkflow = useAddMaterialToWorkflow();
  ...
  const handleAddToExistingWorkflow = async (workflowId: string, workflowTitle: string) => {
    ...
    try {
      await addMaterialToWorkflow.mutateAsync({ workflowId, materialId: material.id });
      toast.success(`"${material.title}" has been added to "${workflowTitle}"`);
      onClose();
    } catch (error: any) {
      toast.error(error.message || 'Failed to add material to workflow.');
                  ...
                  {filteredWorkflows.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      {existingWorkflows.length === 0 
                        ? "No active workflows found. Create a new one."
